...
def plot_metrics_comparison(results: dict, save: bool = True) -> plt.Figure:
    ...
    for i, model in enumerate(models):
        ...
        bars = ax.bar(x + i * width, vals, width, label=model, color=COLORS[model], alpha=0.88, zorder=3)
        for bar, v in zip(bars, vals):
            ax.text(bar.get_x() + bar.get_width() / 2, bar.get_height() + 0.004, f"{v:.3f}", ha="center", va="bottom", fontsize=7.5)
...
def plot_holiday_vs_regular(results: dict, save: bool = True) -> plt.Figure:
    ...
    b1 = ax.bar(x - width / 2, reg, width, label="Regular weeks", color=[COLORS[m] for m in models], alpha=0.88, zorder=3)
    b2 = ax.bar(
        x + width / 2,
        hol,
        width,
    ...
    for bars in (b1, b2):
        for bar in bars:
            v = bar.get_height()
            ax.text(bar.get_x() + bar.get_width() / 2, v + 0.003, f"{v:.3f}", ha="center", va="bottom", fontsize=8)
...
def plot_wce(results: dict, save: bool = True) -> plt.Figure:
    ...
    for bar, v in zip(bars, vals):
        ax.text(bar.get_x() + bar.get_width() / 2, v + 0.003, f"{v:.4f}", ha="center", va="bottom", fontsize=10)
...
def plot_feature_importance_grouped(results: dict, save: bool = True) -> plt.Figure:
    ...
    for i, model in enumerate(models):
        ...
        ax.barh(y + i * height, vals, height, label=model, color=COLORS[model], alpha=0.88, zorder=3)
...
def plot_feature_importance_per_model(results: dict, save: bool = True) -> plt.Figure:
    ...
    if save:
        fig.savefig(FIGURES_DIR / "06_feature_importance_per_model.png", dpi=150, bbox_inches="tight")
    return fig
