    ...
    for col in REQUIRED_COLUMNS:
        if col not in df.columns:
            checks.append({
                ...
                "column": col, "type": "column_missingness",
                "status": "FAIL", "note": "Column missing from dataset",
            })
            continue
        ...
        criticality = (
            "critical" if max_null == 0.0 else
            "high" if max_null <= 5.0 else "medium"
        )
        checks.append({
